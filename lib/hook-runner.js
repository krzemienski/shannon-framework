...
// Project-level opt-in. Shannon hooks are SILENT no-ops unless the project
// explicitly opts in. This prevents governance from polluting unrelated repos.
//   opt-in  : <project>/.shannon/active  (sentinel file)
//   opt-out : <project>/.shannon/disabled OR env SHANNON_DISABLE=1
//   global  : env SHANNON_GLOBAL=1 forces active everywhere (escape hatch)
function projectRoot() {
  return process.env.CLAUDE_PROJECT_DIR || process.cwd();
}

function isShannonActive() {
  ...
  try {
    ...
    return false; // fail-closed: never enforce when uncertain
  }
}
...
function runHook(scriptName, handler, opts) {
  const start = Date.now();
  let payload = {};
  let result = { decision: 'allow', exitCode: 0, stderrPayload: '' };
  const options = opts || {};

  // Opt-in gate: silent no-op in projects that have not enabled Shannon.
  // `alwaysRun` hooks (rare) bypass the gate.
  if (!options.alwaysRun && !isShannonActive()) {
    process.exit(0);
    return;
  }
  ...
  if (result.stdoutPayload) process.stdout.write(String(result.stdoutPayload));
