#!/usr/bin/env python3
"""
Shannon v5.0 - Tier 2: Build Repo Nexus iOS App

Builds complete Repo Nexus full-stack iOS application and verifies.

Specification: docs/ref/repo-nexus-spec.md (87KB)
Expected: React Native + FastAPI + PostgreSQL + Redis
...
BUILD_CONFIG = {
    'name': 'Repo Nexus',
    'spec_file': 'docs/ref/repo-nexus-spec.md',
    'output_dir': 'test-builds/repo-nexus',
    'expected_files': [
        'package.json',
        'app.json',
        'README.md',
        ...
        'src/screens/RepositoriesScreen.tsx',
        'backend/main.py',
...
async def build_application(spec_text: str, output_dir: Path) -> tuple[bool, str]:
    ...
    print(f"Building full-stack iOS application with Shannon...")
    ...
    prompt = f'''Build the complete full-stack iOS application from this specification.
...
Execute /sh_wave to build everything including mobile app, backend API, and database.
'''

    print(f"\nExecuting /sh_wave (this will take 60-75 minutes)...")
    print("Progress:")
...
async def run_verification_skill(output_dir: Path, spec_text: str, skill_path: Path) -> tuple[bool, str]:
    ...
    prompt = f'''Verify this built full-stack iOS application meets the specification requirements.
