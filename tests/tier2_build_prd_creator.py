#!/usr/bin/env python3
"""
Shannon v5.0 - Tier 2: Build PRD Creator

Builds complete PRD Creator application and verifies with shannon-execution-verifier skill.

Specification: docs/ref/prd-creator-spec.md (18KB)
Expected: Web app (React + FastAPI + PostgreSQL)
...
BUILD_CONFIG = {
    ...
    'output_dir': 'test-builds/prd-creator',
    'expected_files': [
        'package.json',
        'requirements.txt',
        'README.md',
...
async def build_application(spec_text: str, output_dir: Path) -> tuple[bool, str]:
    ...
    """
    print(f"Building application with Shannon...")
    ...
    # Build prompt
    prompt = f'''Build the complete application from this specification.
...
def verify_file_structure(output_dir: Path, expected_files: list) -> tuple[bool, list]:
    ...
    Returns: (success: bool, missing_files: list)
    """
...
async def run_verification_skill(output_dir: Path, spec_text: str, skill_path: Path) -> tuple[bool, str]:
    """
    Execute shannon-execution-verifier skill on built application
    ...
    prompt = f'''Verify this built application meets the specification requirements.
...
'''
    ...
    # Look for indicators like "✅", "PASSED", "All checks passed"
...
async def main():
    ...
    print(f"\nLoaded specification: {file_size_kb:.1f} KB")
    ...
    if not success:
        ...
        print(f"   Application incomplete")
        return 1

    # Step 4: Run comprehensive verification skill
