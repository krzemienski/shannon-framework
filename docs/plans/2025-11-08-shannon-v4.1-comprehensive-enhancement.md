...
**Files:**
- Read: `skills/spec-analysis/SKILL.md` (currently 1,242 lines)
- Modify: `skills/spec-analysis/SKILL.md:1243-end`
...
```bash
Read skills/spec-analysis/SKILL.md
```
Expected: 1,242 lines, verify has examples, validation section, common pitfalls

**Step 2: Validate feasibility**
- Check: Can Claude actually execute 8D algorithm? ✅ (regex parsing, math calculations)
- Check: Are dimension weights correct? ✅ (sum to 100%)
- Check: Is domain detection feasible? ✅ (keyword matching)
...
- Has: Examples ✅, pitfalls ✅, validation ✅
...
**Step 5: Commit**
```bash
git add skills/spec-analysis/SKILL.md
...
**Duration**: 20-30 minutes per skill × 14 skills = 4-7 hours

### Task 16: skill-discovery Enhancement

**Files:**
- Modify: `skills/skill-discovery/SKILL.md`

**Status**: Already enhanced (added 3 examples in previous work)

**Step 1: Verify enhancement complete**
```bash
grep -c "^### Example" skills/skill-discovery/SKILL.md
```
...
**Files:**
- Create: `hooks/SESSIONSTART_HOOK.md`
- Read: `hooks/session_start.sh`
- Read: `hooks/hooks.json:61-74`
...
echo "🎯 Shannon Framework V4.1 initializing..."
...
├── SessionStart Hook (FOUNDATION)
│   └─ Loads using-shannon meta-skill
│       ├─ Enforces 8D analysis
│       ├─ Enforces NO MOCKS
│       ├─ Enforces wave execution
│       └─ Enforces context preservation
├── PreCompact Hook (context preservation)
├── PostToolUse Hook (NO MOCKS enforcement)
├── Stop Hook (wave validation)
└── UserPromptSubmit Hook (North Star injection)
```
...
```bash
Write hooks/SESSIONSTART_HOOK.md
```

**Step 5: Commit**
```bash
git add hooks/SESSIONSTART_HOOK.md
...
**Files:**
- Create: `hooks/PRECOMPACT_HOOK.md`
- Read: `hooks/precompact.py`
- Read: `hooks/hooks.json:17-30`
...
- /shannon:restore or /shannon:prime restores 100%
...
7. Verify save successful (read-back test)
   ↓
8. Report: "✅ Checkpoint created: shannon_checkpoint_{timestamp}"
   ↓
...
**Checkpoint created → Restored via**:

**Method 1**: /shannon:restore command
```bash
...
```
Context Growth:
0 tokens → 100K → 500K → 900K (PreCompact fires) → Save checkpoint → 1M (compact) → 200K (post-compact)

WITHOUT PreCompact:
900K tokens of context → LOST
User must re-explain everything

WITH PreCompact:
900K tokens → Checkpoint saved to Serena
Post-compact: /shannon:restore → 100% restoration
```
...
Checkpoint restored:
   ✅ Spec loaded
   ✅ Wave 1 deliverables known
   ✅ Wave 2 progress: 65% (Stripe next)
...
**Step 4: Commit**
```bash
git add hooks/PRECOMPACT_HOOK.md
...
**Files:**
- Create: `hooks/POSTTOOLUSE_HOOK.md`
- Read: `hooks/post_tool_use.py`
- Read: `hooks/hooks.json:32-45`
...
**Step 5: Commit**
```bash
git add hooks/POSTTOOLUSE_HOOK.md
...
**Files:**
- Create: `hooks/USERPROMPTSUBMIT_HOOK.md`
...
- 🏥 Healthcare (HIPAA, patient safety, medical accuracy)
...
**Other Frameworks**:
- "Looks complex" (subjective) → Shannon: 0.68/1.0 (quantitative 8D score)
- Manual skill checklists (30% forgotten) → Shannon: 100% auto-discovered
- 6 commands, 15-20 minutes resumption → Shannon: 1 command, <60 seconds
- Optional reading guidelines → Shannon: Architectural enforcement (FORCED_READING_PROTOCOL)
- Mocks allowed → Shannon: NO MOCKS iron law (functional tests only)
...
**If Error**:
- "Serena MCP not connected" → See Step 7 (Configure Serena)
- "Plugin not found" → Check marketplace path (use absolute path)
- "Commands not available" → Ensure complete restart
...
📚 Step 1/8: Discovering skills...
   ✅ 104 skills discovered (16 project, 88 user)

🔌 Step 2/8: Verifying MCPs...
   ✅ Serena MCP: Connected
   ✅ Sequential MCP: Available
   ⚠️  Puppeteer MCP: Not connected (optional)

🧠 Step 3/8: Preparing Sequential thinking...
   ✅ Ready for complex analysis

🔍 Step 4/8: Enabling forced reading...
   ✅ FORCED_READING_PROTOCOL active

✅ Step 8/8: Session ready!

═══════════════════════════════════════
🎯 SHANNON FRAMEWORK V4.1 - SESSION READY
═══════════════════════════════════════
...
```bash
/shannon:spec "Build a simple contact form"
```

**Expected**: 8D complexity analysis, domain breakdown, recommendations

**If Successful**: Shannon V4.1 is installed and working! ✅
...
### Task 23: /shannon:spec Usage Guide

**Files:**
- Create: `commands/guides/sh_spec_GUIDE.md`
...
### ❌ Anti-Pattern 3: Missing Key Information
### ❌ Anti-Pattern 4: Accepting User's Score

## Recommended Alternatives
[For each anti-pattern, provide correct approach]

## Integration
[How /shannon:spec works with other commands]
```
...
**Step 4: Commit**
```bash
mkdir -p commands/guides
git add commands/guides/sh_spec_GUIDE.md
git commit -m "docs(commands): add comprehensive /shannon:spec usage guide with 15 examples"
```

### Tasks 24-30: Additional Priority Command Guides

**Task 24**: /shannon:prime guide (10-15 examples)
**Task 25**: /shannon:wave guide (10-15 examples)
**Task 26**: /shannon:discover_skills guide (10-15 examples)
**Task 27**: /shannon:checkpoint guide (10-15 examples)
**Task 28**: /shannon:restore guide (10-15 examples)
**Task 29**: /shannon:test guide (10-15 examples)
**Task 30**: /shannon:analyze guide (10-15 examples)
...
**Files:**
- Read: `skills/using-shannon/SKILL.md`
...
Analysis: ✅ Agent followed using-shannon despite maximum pressure
```
...
- RED (no skill): Chose tests-after ❌
- GREEN (with skill): Chose TDD restart ✅
- Effectiveness: ✅ BULLETPROOF
...
**Files:**
- Read: `hooks/hooks.json`
- Modify: `hooks/hooks.json` (if changes needed)
...
All 5 hooks:
✅ SessionStart - Configured, tested, working
✅ PreCompact - Configured, tested, working
✅ PostToolUse - Configured, tested, working
✅ Stop - Configured, tested, working
✅ UserPromptSubmit - Configured, tested, working
...
- [ ] Priority command guides created (4-8 commands × 10-15 examples)
